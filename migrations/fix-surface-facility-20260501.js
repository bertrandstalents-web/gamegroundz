...
async function runMigration() {
    ...
    db.run(`UPDATE surfaces SET facility_id = 18 WHERE name LIKE '%arena st canut%' AND facility_id = 17`, [], function(err) {
        if (err) {
        ...
        db.all(`SELECT id, name, facility_id FROM surfaces WHERE name LIKE '%arena st canut%'`, [], (err, rows) => {
            if (err) {
